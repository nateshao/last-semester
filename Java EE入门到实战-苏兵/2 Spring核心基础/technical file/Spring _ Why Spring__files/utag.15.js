...
s.linkInternalFilters="javascript:,vmware.com,vmworld.com,vforum.jp,vmware-vforum.com,vmware-juku.jp,whatisworkspaceone.com";
...
//tealium universal tag - utag.sender.19063.am141 v4.0.202108030628, Copyright 2021 Tealium.com Inc. All Rights Reserved.
try{
	...
	sc_sendForm = function(formname, defcid, rNumb) {
		...
		*/
		...
		if(email != '') {
			...
			s.eVar42 = jQuery.sha256(email);
			s.linkTrackVars = s.linkTrackVars + ',eVar42';
			if(((window.location.host !== "events.vmware.com" || window.location.host !== "events-dev.vmware.com" || window.location.host !== "events-uat2.vmware.com" ||window.location.host !== "events-stg.vmware.com"|| window.location.host !== "localhost:8080" || window.location.host !== "events.vmware.com") &&( window.location.href.indexOf("/web/registration") < 0 || (location.href.toLowerCase().indexOf("sase-review.vmware.com/resources") < 0 || location.href.toLowerCase().indexOf("sase.vmware.com/resources") < 0)))){
			visitor.setCustomerIDs({
				"hashedemail": {
					"id": jQuery.sha256(email),
					"authState": Visitor.AuthState.AUTHENTICATED
		...
		if(varCid != '' || varSrcId != '') {
			...
			if((location.href.toLowerCase().indexOf("sase-review.vmware.com/resources") > -1 || location.href.toLowerCase().indexOf("sase.vmware.com/resources") > -1) && location.href.toLowerCase().indexOf('-ty-')<0 &&  location.href.toLowerCase().indexOf('/ty')<0 && location.href.toLowerCase().indexOf('thank-you') < 0){
...
checkInternalTraffic = function() {
		//Check if we are on internal domain
		...
		var isvmware = null;
		if(!(typeof dbInfo.company_name === "undefined" || dbInfo.company_name === null)) {
			isvmware = dbInfo.company_name.match(/vmware/i);
		}
		if((isInternalDomain || isvmware != null) && cinternal == "") {
			var t = new Date();
		...
		if(isInternalDomain) {
			s.account = s.prop75 = "vmwaredev";
		}
		if(cinternal != "" && !isInternalDomain) {
			s.prop70 = 1;
			s.account = s.prop75 = "vmwareinternaldata"; // Set account Name to Internal traffic suite
		}
	}
	//set cookie function
...
$(document).on("keypress","form input[type!=hidden]",function(){
...
if((location.href.toLowerCase().indexOf("sase-review.vmware.com/resources") > -1 || location.href.toLowerCase().indexOf("sase.vmware.com/resources") > -1) && location.href.toLowerCase().indexOf('-ty-')<0 && location.href.toLowerCase().indexOf('/ty')<0 && location.href.toLowerCase().indexOf('thank-you') < 0 && jQuery('form').length > 0){
var saseFormInputId = "";
var saseFormAbon = 1;
 ...
 */
	...
	if(location.href.toLowerCase().indexOf("vmworld-2020-registration-is-now-open/") > -1) {} else if((location.href.toLowerCase().indexOf("contact-us") > -1 && (document.URL.indexOf('scp=true') == -1) || location.href.toLowerCase().indexOf("registration") > -1 || location.href.toLowerCase().indexOf("register") > -1 || location.href.toLowerCase().indexOf("request-an-invitation-to-futurenet") > -1 || location.href.toLowerCase().indexOf("futurenet") > -1 && $('form#fnet_contact').length > 0) || ((location.href.toLowerCase().indexOf("sase-review.vmware.com/resources") > -1 || location.href.toLowerCase().indexOf("sase.vmware.com/resources") > -1) && location.href.toLowerCase().indexOf('/ty')<0 && location.href.toLowerCase().indexOf('-ty-')<0 && location.href.toLowerCase().indexOf('thank-you') < 0 && jQuery('form').length > 0) ) {
		s.events = s.apl(s.events, "event43");
		var varV17 = s.pageName;
		var arrForm = s.formList.split(',');
		if(location.href.toLowerCase().indexOf("contact-us") > -1 && (document.URL.indexOf('scp=true') == -1) || location.href.toLowerCase().indexOf("register") > -1 || ((location.href.toLowerCase().indexOf("sase-review.vmware.com/resources") > -1 || location.href.toLowerCase().indexOf("sase.vmware.com/resources") > -1) && location.href.toLowerCase().indexOf('-ty-')<0 && location.href.toLowerCase().indexOf('/ty')<0 && location.href.toLowerCase().indexOf('thank-you') < 0 && jQuery('form').length > 0)) {
			//s.eVar17 = varV17 + ':' + arrForm[0];
			s.eVar17 = varV17 + ' : TheForm';
			if(location.href.toLowerCase().indexOf("sase-review.vmware.com/resources") > -1 || location.href.toLowerCase().indexOf("sase.vmware.com/resources") > -1){
				saseForm = 1;
			}
			...
			} else if($("#resendEmailForm").length > 0) {
		...
		} else if(location.href.toLowerCase().indexOf("futurenet") > -1 && $('form#fnet_contact').length > 0) {
	...
	/* formAnalysis */
	if(location.href.toLowerCase().indexOf("sase-review.vmware.com/resources") < 0 && location.href.toLowerCase().indexOf("sase.vmware.com/resources") < 0){
	s.setupFormAnalysis();
	}
	/* form pages */
...
*/
	...
	//Below logic added to set name attribute in button tags for Activity Map Link Tracking - Start
	
    jQuery(document).on('mouseover', 'body button', function() {
		if(typeof(jQuery(this).attr('name')) == 'undefined' || jQuery(this).attr('name').length == 0) {
			lp = "&lpos=apps_scodevmw : " + jQuery('body button').index(jQuery(this));
			jQuery(this).attr('name', lp);
		}
	});
	
	//Below logic added to set name attribute in button tags for Activity Map Link Tracking - End
	...
	//Logic for setting the s_objectID in All links for all Activity Map Link Tracking - START

	jQuery(document).on('mouseover', 'body a, body button', function() {
